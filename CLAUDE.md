...
La **gestión de evidencias** es un ciclo entre director y docente: el director solicita un entregable concreto (`improvement_plan_evidence_requests`), el docente sube el archivo, ambos comentan en el hilo de la solicitud (`improvement_plan_evidence_comments`) y el director aprueba o rechaza. Una entrega pasa la solicitud a `EN_REVISION`; un rechazo la devuelve a `PENDIENTE` y deja un comentario de sistema para que el docente reenvíe. Cada transición notifica a la otra parte con `NotificationService` usando `link` para enlazar al plan. Ese aviso sale **aunque quien actúa sea el destinatario**: una misma cuenta puede ser las dos partes (un director con un plan a su propio nombre), y callar la notificación ahí dejaba ese plan sin rastro del ciclo — que es justo el caso con el que se prueba el flujo de punta a punta. Lo mismo vale para el correo (`_send`) y para el aviso de formato firmado (`_announce_signed`).

La **verificación** es lo que dice el semestre siguiente sobre un plan ya cerrado. El cierre ocurre al firmar el Formato 3, antes de que existan las notas que probarían la mejora, así que no puede ser el momento de la comparación: `api/utils/plan_verification.py` la corre sola desde `evaluation_processor`, en dos pasadas — los indicadores al terminar el procesamiento (`verify_scores_for_evaluation`) y los comentarios al terminar el análisis de IA (`verify_comments_for_evaluation`, que necesita nivel de riesgo y categoría pedagógica). El veredicto es el promedio del docente sobre **todos** sus grupos del periodo, que es lo que pactó el acta; el desglose por asignatura del mismo indicador se guarda aparte para ver el caso en que la meta se alcanza en general pero una materia sigue baja. En los ítems cualitativos solo `ALTO` levanta alerta y solo si el comentario reaparece en la **misma categoría pedagógica** que motivó el compromiso; `MEDIO` se guarda como contexto. Un plan cuyos compromisos son **todos** cualitativos no tiene número que lo juzgue, así que ahí la reincidencia sí es el veredicto (`_qualitative_result`); en cuanto el acta pactó una meta medible, mandan las notas y un comentario que reaparece no deshace una meta alcanzada. El hallazgo guarda una copia del nivel de riesgo y la categoría que tenía el comentario al verificar, así que re-etiquetarlo a mano vuelve a correr la segunda pasada (`reverify_comment`, enganchada en `CommentService.update_classification`) para que el plan y el comentario nunca digan cosas distintas; ahí no se notifica al director, que es quien acaba de hacer el cambio. Todo va a `improvement_plan_verifications` (+ `_items`, `_courses`, `_comments`) y **nunca toca el cierre firmado**: `plan.status` es lo que el director firmó, `verification.result` lo que dijeron las notas.
